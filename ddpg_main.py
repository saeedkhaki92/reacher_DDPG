...
UPDATE_EVERY = 18       # timesteps between updates
NUM_UPDATES = 14        # num of update passes when updating
...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed=23,
                 fc1_units=100, fc2_units=100, epsilon=1.0, lr_actor=1e-3,
    ...
    def recreate(self):
        self.actor = Actor(self.state_size, self.action_size, self.random_seed).to(device)
        self.actor_target = Actor(self.state_size, self.action_size, self.random_seed).to(device)
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=self.lr_actor)

        self.critic = Critic(self.state_size, self.action_size, self.random_seed).to(device)
    ...
    def step(self,buffers,states, actions, rewards, next_states, dones,t):
        store_function(buffers, states, actions, rewards, next_states, dones, t)
        #learn_function(agent, buffers, t)
        
        memory, cache = buffers
        if len(memory) > int(BUFFER_SIZE) and t % UPDATE_EVERY == 0:
...
def store_function(buffers, states, actions, rewards, next_states, dones, timestep):
    memory, cache = buffers
...
def learn_function(agent, buffers, timestep):
    memory, cache = buffers
